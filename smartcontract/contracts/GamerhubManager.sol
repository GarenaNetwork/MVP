...
contract GamerhubManager {
   ...
   /** @dev get details for different packages.
     ...
     * @param _package  The selected package.
     */
    function subscribe (uint _package) external payable {
        //validate amount sent
        require(msg.value == packages[_package].amount, "Invalid amount paid");
        
        uint start = block.timestamp;
        uint end =  start + packages[_package].duration;
        //record subscription
        subscriptions[msg.sender] = Subscription(_package,start,end);
        emit subscribed(msg.sender, _package, start, end);
    }
